...
// imports from src/ - see convex/infinidraft/draft/tiers.ts's comment on the same
...
// just the overall rank (convex/ never imports from src/ - see convex/infinidraft/draft/
// tiers.ts's comment on the same duplication convention; convex/infinidraft/gemini/
...
async function computeReportCardData(
  ...
  // Mirrors convex/infinidraft/gemini/preDraftInsights.ts's snake branch and src/lib/
...
// *Public queries (see convex/leagues.ts's getSeasonPublic, convex/infinidraft/draft/
...
// This mirrors how the AI recap (convex/infinidraft/gemini/reportSummary.ts) already
...
export const getDraftReportCardPublic = query({
  ...
  handler: async (ctx, args) => {
    ...
    // AI-written recap (+ per-team blurbs), generated once by convex/infinidraft/gemini/
    // reportSummary.ts's generateReportSummary action when the draft first
    // completes - see convex/infinidraft/draft/status.ts. Only the generated text is
...
// Internal counterpart of getDraftReportCardPublic, for convex/infinidraft/gemini/
...
// Backfill trigger for the AI recap - convex/infinidraft/draft/status.ts only ever
...
// one that came back truncated - see convex/infinidraft/gemini/client.ts's
