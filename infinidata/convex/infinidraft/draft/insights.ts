...
// Card - see convex/infinidraft/draft/reportCard.ts's getDraftReportCardPublic comment
...
// Frontend-triggered backfill, same convention as convex/infinidraft/draft/reportCard.ts's
...
export const ensureInsightsGenerated = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.scheduler.runAfter(
      0,
      internal.infinidraft.gemini.preDraftInsights.generatePreDraftInsights,
      args,
    );
  },
});
...
// convex/infinidraft/draft/reportCard.ts's regenerateReportSummary. Throws for a non-Pro
