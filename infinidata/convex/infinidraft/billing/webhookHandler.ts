...
// over the raw, timestamped payload) - everything else in convex/infinidraft/billing/
...
export const processStripeWebhookEvent = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const isNewEvent = await ctx.runMutation(
      internal.infinidraft.billing.mutations.claimWebhookEvent,
    ...
    switch (event.type) {
      ...
      case "customer.subscription.deleted": {
        const subscription = event.data.object as Stripe.Subscription;
        await ctx.runMutation(internal.infinidraft.billing.mutations.markSubscriptionCanceled, {
          stripeSubscriptionId: subscription.id,
        });
        break;
      }
