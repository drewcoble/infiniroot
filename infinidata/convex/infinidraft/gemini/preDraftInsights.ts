...
// and convex/infinidraft/draft/insights.ts's getPreDraftInsights (recomputes it on every
...
// never imports from src/ - see convex/infinidraft/draft/tiers.ts's comment on the same
...
// convex/infinidraft/gemini/reportSummary.ts's buildSummaryPrompt. Returns null when
...
// convex/infinidraft/gemini/reportSummary.ts's RESPONSE_SCHEMA for the same pattern.
...
// Scheduled by convex/infinidraft/draft/insights.ts's ensureInsightsGenerated/
// regenerateInsights - best-effort, same resilience posture as
// convex/infinidraft/gemini/reportSummary.ts's generateReportSummary: any failure
...
export const generatePreDraftInsights = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const alreadyGenerated: boolean = await ctx.runQuery(
      internal.infinidraft.gemini.preDraftInsights.hasCachedInsights,
      { seasonId: args.seasonId, week: args.week, scoring },
    );
    if (alreadyGenerated) return;

    const inputs = await ctx.runQuery(
      internal.infinidraft.gemini.preDraftInsights.gatherInsightsInputs,
      args,
    );
    if (!inputs) return;
    ...
    await ctx.runMutation(internal.infinidraft.gemini.preDraftInsights.saveInsights, {
      seasonId: args.seasonId,
      week: args.week,
      scoring,
      insights,
