...
// uses (see convex/draft/reportCard.ts's ensureReportSummaryGenerated).
...
// Best-effort, matching convex/gemini/reportSummary.ts's generateReportSummary
...
export const fetchAndCachePricing = internalAction({
  ...
  handler: async (ctx) => {
    ...
    await ctx.runMutation(internal.billing.pricing.savePricing, {
...
export const ensureProPricingCached = mutation({
  ...
  handler: async (ctx) => {
    ...
    await ctx.scheduler.runAfter(
      0,
      internal.billing.pricing.fetchAndCachePricing,
      {},
    );
  },
});
