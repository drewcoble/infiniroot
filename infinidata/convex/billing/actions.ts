...
export const startCheckout = action({
  ...
  handler: async (ctx, args): Promise<{ url: string }> => {
    ...
    const [subscription, currentUser] = await Promise.all([
      ctx.runQuery(internal.billing.queries.getSubscriptionRow, { userId }),
...
export const openBillingPortal = action({
  ...
  handler: async (ctx, args): Promise<{ url: string }> => {
    ...
    const subscription = await ctx.runQuery(
      internal.billing.queries.getSubscriptionRow,
      { userId },
    );
...
// webhook to arrive - the webhook (convex/billing/webhookHandler.ts) remains
