...
export const startYahooAuth = action({
  ...
  handler: async (ctx, args): Promise<{ authorizeUrl: string }> => {
    ...
    await ctx.runMutation(internal.yahoo.oauth.createOAuthState, {
      state,
      userId,
...
// Every Yahoo API call in convex/yahoo/league.ts goes through this so token
...
export async function withYahooToken<T>(
  ...
  const stored = await ctx.runQuery(internal.yahoo.oauth.getStoredToken, {
    userId,
  });
  if (!stored) {
    throw new Error(
