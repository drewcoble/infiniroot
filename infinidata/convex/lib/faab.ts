...
import { pointsForScoringConfig, type ScoringConfig, scoringConfigFromSeason } from "../scoring";
...
// Same falloff-curve idea as convex/infinidraft/values/draftValues.ts's
// FALLOFF_EXPONENT (raw VOR over-concentrates $ at the top of the pool) - a
// separate tunable constant since FAAB bidding behavior isn't the same
// market as a draft auction; starts at the same value pending real bid data
// to retune against.
...
// convex/infinidraft/values/draftValues.ts's computeDraftValues - same
// non-flex/FLEX/SUPERFLEX demand tiering, but fed the free-agent-only pool
// ranked by rest-of-season value instead of the full draft pool ranked by
// season points. Demand per position is approximated as teamCount *
// rosterSlots[pos] (the draft engine's own simplification), rather than
// trying to reconstruct exactly how many teams have a genuinely open/needy
// slot at this position this week - a reasonable starting heuristic, retune
// later if suggestions feel off in a shallow position.
