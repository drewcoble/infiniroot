...
// infinidraft/infinileague's AuthPanel: this account and the other two
// apps' are the same authAccounts row (shared Convex deployment), so this
// hits the exact same email-enumeration risk.
...
function toFriendlySignUpMessage(rawMessage: string): string {
  ...
  if (ACCOUNT_ALREADY_EXISTS_PATTERN.test(rawMessage)) {
    return "Couldn't create an account with those details. Double-check the email, or try signing in instead - if you already have an infinidraft/infinileague account, use that same email and password here.";
  }
